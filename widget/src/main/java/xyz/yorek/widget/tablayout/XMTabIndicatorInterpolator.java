...
/**
 * A class used to manipulate the {@link XMTabLayout.SlidingTabIndicator}'s indicator {@link Drawable} at any
 * point at or between tabs.
 *
 * <p>By default, this class will size the indicator according to {@link
 * XMTabLayout#isTabIndicatorFullWidth()} and linearly move the indicator between tabs.
 *
 * <p>Subclasses can override {@link #setIndicatorBoundsForTab(XMTabLayout, View, Drawable)} and
 * {@link #setIndicatorBoundsForOffset(XMTabLayout, View, View, float, Drawable)} (TabLayout, View,
...
class XMTabIndicatorInterpolator {
  ...
  static RectF calculateTabViewContentBounds(
    ...
    int minWidthPx = (int) XMViewUtils.dpToPx(tabView.getContext(), minWidth);
   ...
   * selected.
   *
   * <p>This method accounts for {@link XMTabLayout#isTabIndicatorFullWidth()}'s value. If true, the
   ...
   *
   * @param tabLayout The tab's parent {@link XMTabLayout}
   ...
   *
   * @param tabLayout The {@link XMTabLayout} parent of the tab and indicator being drawn.
  ...
  void setIndicatorBoundsForOffset(
    ...
    indicator.setBounds(
        XMAnimationUtils.lerp((int) startIndicator.left, (int) endIndicator.left, offset),
        indicator.getBounds().top,
        XMAnimationUtils.lerp((int) startIndicator.right, (int) endIndicator.right, offset),
        indicator.getBounds().bottom);
  }
}
...
/**
 * An implementation of {@link XMTabIndicatorInterpolator} that translates the left and right sides of
 * a selected tab indicator independently to make the indicator grow and shrink between
 * destinations.
 */
class XMElasticTabIndicatorInterpolator extends XMTabIndicatorInterpolator {
  ...
  void setIndicatorBoundsForOffset(
      ...
      indicator.setBounds(
          XMAnimationUtils.lerp((int) startIndicator.left, (int) endIndicator.left, leftFraction),
          indicator.getBounds().top,
          XMAnimationUtils.lerp((int) startIndicator.right, (int) endIndicator.right, rightFraction),
